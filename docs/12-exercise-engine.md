...
| 19 | Role-play | ✅ **correção de 2026-08-28**: a auditoria anterior (mesma data) tinha afirmado erradamente que este tipo "não tem cenários distintos" — inspeção direta confirma o contrário: `/speak/page.tsx` tem 4 cenários reais (restaurante/hotel/aeroporto/reunião) via `ROLEPLAY_SCENARIOS`, cada um com o seu próprio system prompt (`SCENARIO_FOCUS` em `/speak/tutor/page.tsx`) que chega genuinamente ao servidor via `sessionFocus` (`api/ai/tutor/route.ts:44-45,82`) e molda a personagem que o Gemini encarna. Reutiliza a mesma UI/avaliação de fim de conversa do tipo 9 (decisão de arquitetura razoável, não uma lacuna) mas o CONTEÚDO é genuinamente distinto por cenário |
| 20 | Desafio de escrita livre | ✅ **novo** (`/practice/writing-challenge`), formato ❌⚠️✅ + "como um nativo escreveria" |
| 21 | Desafio de Discurso Livre (não fazia parte da lista original) | ✅ **novo (auditoria 2026-08-28, achado crítico)** — (`/practice/speaking-challenge`) 45-90s de fala contínua e não ensaiada sobre um tema, transcrita ao vivo (`SpeechRecognition` `continuous`+`interimResults`), avaliada por Gemini (grammar/vocabulary/coherence/fluency) combinado com WPM real medido no servidor. Fecha o degrau que faltava entre "conversa curta" e "conversação real" na escada Repeat→Controlled→Short→Open→**Extended**→Conversation→Spontaneous |

**21/21 confirmados funcionais** (8 já existiam antes da 1ª ronda + 12 genuinamente novos: Correção de Erros, Sinónimos/Antónimos, Contexto, avaliação de Conversação com IA, Tradução EN→PT, Word Builder, Desafio de Escrita com formato ❌⚠️✅, Leitura em Voz Alta, Challenge/Apply do Quiz de Gramática, Preencher Espaços, Ouvir e Escolher com progressão por camada, Desafio de Discurso Livre). **1 vídeo genuinamente bloqueado** por falta de infraestrutura de vídeo — zero conteúdo, zero streaming, não viável a custo zero. Todos os tipos funcionais partilham o mesmo Exercise Engine (`src/lib/exercise/`), a mesma integração com progresso/gamificação, e a mesma consistência visual (`ExerciseShell`/padrão writing-challenge).
