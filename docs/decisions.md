...
**Achado real, não cosmético**: `PILLAR_ACCENT` (`src/lib/pillarDisplay.ts`) só cobria 5 dos 8 pilares, com pares indistinguíveis (GRAMMAR=READING=verdigris, VOCABULARY=TRANSLATION=brass) e — mais grave — **LISTENING pedia Clay emprestado**, a cor que `docs/09-sistema-design.md` reserva explicitamente e só a marcação de erros PT→EN ("Clay é reservado a erros de transferência PT→EN em todo o produto... nunca usada decorativamente"). Em `home/page.tsx`/`practice/page.tsx` as cores dos ~24 cards eram escolhidas à mão por posição (brass/verdigris/clay alternados), não pelo mapa central — confirmado por leitura de código e por screenshots reais do site publicado (Home/Practice visualmente corretos mas monótonos: todos os cards com a mesma forma, só a cor do texto do rótulo variava).

**Corrigido**: 6 cores novas (`moss`/`teal`/`slate`/`indigo`/`plum`/`berry`, `tailwind.config.ts`), mesma família tonal muted/editorial das existentes — cada um dos 8 pilares passa a ter cor própria, Clay volta a ser exclusivo de erros. Novo `PILLAR_ICON` (`pillarDisplay.ts`) + `PillarIcon.tsx` (SVG inline desenhado à mão, mesmo espírito do `SkillOctagon.tsx` já existente — sem livraria de ícones nova). `Card.tsx` ganha `icon`/`iconClassName` opcionais e retrocompatíveis. `home/page.tsx` e `practice/page.tsx` passam a derivar cor+ícone sempre do pilar real do exercício, em vez de alternância manual — os cards genuinamente cross-pilar (Micro-Desafios, Diagnóstico Semanal, "O que quer trabalhar hoje", Cultura) ficam deliberadamente neutros, não forçados a um pilar que não representam. "Áreas a reforçar"/"Prioridade de hoje" na Home ganham o ícone do pilar ao lado do texto (cor+forma, não só cor — acessibilidade).

**Não implementado nesta ronda, documentado em `docs/13-roteiro-visual-ux.md`**: unificação visual dos ~20 Runners antigos para `ExerciseShell`, objetos interativos (casa/restaurante clicáveis), cenários por profissão, ilustração de personagens no role-play, sistema anti-monotonia dinâmico. Vídeo real fica bloqueado por orçamento, não por esquecimento.

**Verificação em produção (`main@2d30de3`)**: 1º deploy (`main@7ec2c89`) falhou no build — erro de tipos real (`PILLAR_ACCENT`/`PILLAR_ICON` são `Record<string, T>`; com `noUncheckedIndexedAccess`, indexar por variável devolve `T | undefined`, e `PillarLabel`/`pillarHover` em `home/page.tsx`/`practice/page.tsx` usavam o resultado sem `!` nem guarda, ao contrário do padrão já usado no resto do ficheiro). Corrigido e republicado. `/home` e `/practice` confirmados sem erros de consola (só ruído de uma extensão do browser de teste), ícones+cores visíveis e corretos. `/practice/matching` testado interativamente (clique num par correto avançou "0 de 6" → "1 de 6") — funciona, mas revelou um **erro real pré-existente, não introduzido por esta ronda**: React #422/#425 (mismatch de hidratação) ao interagir. Causa: `shuffle()`/`shuffleWords()` usam `Math.random()` dentro de `useMemo`, que corre tanto no render do servidor como na hidratação do cliente com resultados diferentes — confirmado por `git show` que este padrão já existia no `MatchingRunner.tsx` antes de qualquer alteração desta sessão. React recupera sozinho (o exercício continua a funcionar, confirmado pela interação), mas fica registado como achado real para uma ronda futura: mover o `shuffle` para só correr no cliente (`useEffect`+`useState` em vez de `useMemo`, ou `suppressHydrationWarning` no contentor).
