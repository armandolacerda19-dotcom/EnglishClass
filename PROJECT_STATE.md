...
Também: **verbos irregulares** (`src/content/irregularVerbs.ts`, `/practice/verbs`, 99 verbos) e **construção frásica** (`src/content/sentencePatterns.ts`, `/practice/patterns`, 18 padrões de ordem de palavras) — conteúdo estático, mesmo padrão de `readingPassages.ts`/`culturalTips.ts`, sem precisar de schema/seed. `readingPassages.ts` tem 20 textos (Pre-A1 a B1), `idioms.ts` 26 expressões, `culturalTips.ts` 15 dicas — ver `docs/decisions.md` 2026-08-27 para o detalhe da expansão.
