...
import { submitSynonymAntonym } from "@/app/(app)/practice/synonyms/actions";
import type { SynonymAntonymItem } from "@/content/synonymsAntonyms";
import type { GradingResult } from "@/lib/exercise/types";

export function SynonymAntonymRunner({ items }: { items: SynonymAntonymItem[] }) {
  ...
  if (items.length === 0) {
    return (
      <ExerciseShell label="Sinónimos e Antónimos" current={0} total={0}>
  ...
  async function check(option: string) {
    ...
    setSubmitError(null);
    try {
      const res = await submitSynonymAntonym(item.id, option);
      setResult(res);
  ...
  if (done) {
    return (
      <ExerciseComplete
        ...
        title="Sinónimos e Antónimos concluído!"
      >
  ...
  return (
    <ExerciseShell
      label={`Sinónimos e Antónimos · ${item.level}`}
      current={index + 1}
      total={items.length}
    ...
    >
      <p className="mb-1 text-xs font-mono uppercase tracking-wide text-inkNeutral/50 dark:text-linen/50">
        {item.kind === "synonym" ? "Escolha o sinónimo" : "Escolha o antónimo"}
      </p>
      <p className="mb-4 font-display text-2xl">{item.word}</p>
      ...
      {result && (
        <p className="mt-3 text-xs italic text-inkNeutral/60 dark:text-linen/60">{item.translationPt}</p>
      )}
    </ExerciseShell>
  );
}
