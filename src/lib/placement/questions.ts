...
// 5 pilares (grammar/vocabulary/listening/reading/translation) têm 4 perguntas
// (Pre-A1/A1 → A2 → B1 → B2) para estimar um teto aproximado sem exigir um motor
// de item-response completo (esse fica para MVP2, ver docs/10-scope-mvp1.md).
//
...
// — heurística aceitável mas menos precisa. As 5 perguntas B2 abaixo (uma por
// pilar com correção exata) fecham essa lacuna, testando especificamente pontos
// gramaticais que só foram ensinados nos módulos B2 (inversão, causativo/wish
// já eram B1, "should have" e "denied" são B2).
...
export interface PlacementQuestion {
  ...
  difficultyLevel: "PRE_A1" | "A1" | "A2" | "B1" | "B2";
  prompt: string;
  options?: string[];
